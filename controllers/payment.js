...
const initiateEmail = async (id, razorpayPaymentId) => {
  let userData = await COURT_SCHEDULES.findOne({ _id: id })
    .populate("bookedBy")
    .populate("courtId");
  ...
  const info = await transporter.sendMail({
    ...
    html: `<b> Thankyou ${
      bookedBy.fullName
    } for choosing PlEY to book your playground.Your booking has been confirmed with${razorpayPaymentId} for ${
      date.toISOString().split("T")[0]
    } at ${slot} in ${courtId.businessName} . stay fit stay healthy .</b>`,
  });
